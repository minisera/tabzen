...
- **Arc 風 `Ctrl+Tab` 切替**: MRU 順リストを画面中央にオーバーレイ表示。押したまま Tab で移動、Ctrl リリースで確定。`chrome://` など Content Script が動かないページでは `Alt+Q` フォールバックで MRU 2 番目へ即切替
...
- [ ] 任意のウェブページで `Ctrl+Tab` → 中央オーバーレイに MRU 順タブリスト
- [ ] 押したまま Tab 連打で下へ、Shift+Tab で上へ移動
- [ ] Ctrl リリースで選択タブに切替
- [ ] Escape でキャンセル
- [ ] `chrome://extensions/` では標準の Ctrl+Tab (次のタブ) 動作
- [ ] `Alt+Q` がすべてのページ (chrome:// 含む) で動作
